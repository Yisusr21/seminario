...
class clienteapi{
    ...
    public function delete($xdelete =""){
        $aDatos = json_decode($xdelete, true);
        $aResponse = [];
        $sql = "DELETE from cliente ";

        if(strcmp($aDatos["eliminar"],"") !=0 ){
            
            $sql.= "WHERE {$aDatos["eliminar"]} ";
        }
        ...
        $aResponse["estado"]= "success";
        $aResponse["mensaje"]= "cliente numero id: {$aDatos["eliminar"]} se elimino";
        $aResponse["datos"] = $objDB->execute($sql);
